...
static inline bool mesh_send_packet(mesh_packet_t *pkt)
{
    ...
    MESH_DEBUG_PRINTF("SEND %i TO %i THRU %i MAX HOPS %i", pkt->info.pkt_type, pkt->nwk.dst, pkt->mac.dst, pkt->info.hop_count_max);
...
static void mesh_handle_our_packet(mesh_packet_t *pPkt, const bool dup, const bool ack)
{
    ...
    if (!dup && pPkt->info.data_len > 0) {
        MESH_DEBUG_PRINTF("Q PKT FROM %i THRU %i TO %i TYPE %i", pPkt->nwk.src, pPkt->mac.src, pPkt->mac.dst, pPkt->info.pkt_type);
