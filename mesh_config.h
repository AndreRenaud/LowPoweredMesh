 ...
 *
 * Version info :
 *   3  - Each route now has a score, the route will lowest score is removed first.
 ...
 * This is the timeout nodes wait for the destination to send an ACK,
 * otherwise the nodes will repeat the route discovery packet.
 *
 ...
 */
#define MESH_ACK_TIMEOUT_MS          6  ///< Packet is retried if an ACK is not received within this time.
#define MESH_PKT_DISC_TIMEOUT_MS     3  ///< Destined node is given this time before we repeat discovery packet.
#define MESH_RTE_DISCOVERY_HOPS      2  ///< Number of hops to use when a routed packet fails.
/** @} */
...
/**
 * Optionally, define a debug printf() method.
 */
#if 0
    #include <stdio.h>
    #include <time.h>
